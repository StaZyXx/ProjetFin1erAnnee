...
class East(DirectionWrapper):
    ...
    def can_move(self, location: [int, int], id) -> bool:
        ...
        if self.get_game().has_case(x, y + 2):
            ...
            if self.get_game().has_case(x, y + 1) and self.get_game().get_case(x, y + 1).get_case_type() == CaseType.BARRIER:
                return False
    ...
    def can_adapt_for_jump(self, x, y, id):
        if not self.get_game().has_case(x, y + 2):  # Check si il y a une case
            return False
        if not self.get_game().get_case(x, y + 2).check_has_player_without_same_player(id):  # Check si il y a un joueur
            return False
        ...
        if not self.get_game().has_case(x, y + 3):
            return False
        if self.get_game().get_case(x, y + 3).get_case_type() == CaseType.BARRIER:
            return False
